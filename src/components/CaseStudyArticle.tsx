import { ExternalLink, Terminal, Layout, ShieldAlert } from "lucide-react";
import type { CaseStudyWithSlug } from "../lib/projectsData";
...
export default function CaseStudyArticle({ caseStudy }: { caseStudy: CaseStudyWithSlug }) {
  return (
    <article className="relative" id={`case-study-${caseStudy.id}`}>
      {/* Visual Anchor Line */}
          ...
          <div className="pt-4 flex flex-col gap-4">
            {caseStudy.links.map((link, i) => (
              ...
              >
                ...
                {link.icon === "github" ? (
                  <GithubMark className="w-5 h-5 text-[#2EA44F] group-hover:scale-110 transition-transform" />
                ) : (
        ...
        <div className="lg:col-span-8 space-y-16">
          ...
          <div className="space-y-8">
            <section>
              ...
              <p className="text-white/60 leading-relaxed text-sm sm:text-base">{caseStudy.problem}</p>
            </section>
            <section>
              <h3 className="text-lg font-medium mb-3 flex items-center gap-2">
                <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: caseStudy.color }} />
              ...
              <p className="text-white/60 leading-relaxed text-sm sm:text-base">{caseStudy.solution}</p>
            </section>
          </div>
                ...
                <div key={i} className="p-5 rounded-xl bg-white/5 backdrop-blur-xl border border-white/10 shadow-xl flex flex-col justify-center overflow-hidden">
                  <div
                    className={`font-bold mb-2 tracking-tight leading-tight ${
                      metric.value.length > 8 ? "text-xl sm:text-xl md:text-2xl" : "text-2xl sm:text-3xl"
                    }`}
          ...
          <section className="space-y-4">
            ...
            <p className="text-white/60 leading-relaxed text-sm sm:text-base">{caseStudy.businessValue}</p>
          </section>

          {/* Lessons */}
          {caseStudy.lessons && (
            <section className="space-y-4">
              <h3 className="text-lg font-medium">Engineering Lessons</h3>
              <p className="text-white/60 leading-relaxed text-sm sm:text-base">{caseStudy.lessons}</p>
            </section>
          )}

          {/* Future Improvements */}
          {caseStudy.futureImprovements && (
            <section className="border border-dashed border-white/20 p-6 rounded-2xl bg-white/[0.01]">
              <h3 className="font-mono text-xs uppercase tracking-widest text-white/50 mb-2">Future Roadmap</h3>
              <p className="text-white/60 text-sm">{caseStudy.futureImprovements}</p>
            </section>
          )}
        </div>
      </div>
